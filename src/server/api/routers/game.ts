...
export const gameRouter = createTRPCRouter({
  get: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const { week, season } = gameSummary.header as { week: number, season: { type: number } };
      // fetch the game box score
      const boxScoreResponse = await fetch(`${ESPN_BASE_URL}/scoreboard?week=${week}&seasontype=${season.type}`, {
        next: { revalidate: 60 } // revalidate every minute
      });
